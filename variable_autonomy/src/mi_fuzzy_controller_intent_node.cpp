...
std_msgs::Float32 operator_intent_;

ros::NodeHandle n_;
ros::Subscriber loa_sub_,vel_robot_sub_, vel_robot_expert_sub_, sub_goal_status_, intent_sub_;
...
MixedInitiativeControllerDeepgaze::MixedInitiativeControllerDeepgaze(fl::Engine* engine)
{
        ...
        intent_sub_ = n_.subscribe("/operator_intent", 5,&MixedInitiativeControllerDeepgaze::intentCallback, this);
...
void MixedInitiativeControllerDeepgaze::intentCallback(const std_msgs::Float32::ConstPtr& intent)
 {
        operator_intent_ = *intent;
        task_intent_active_ = 1;
        ROS_INFO("Task intent: %d", operator_intent_.data);
        ROS_INFO("task intent active: %d", task_intent_active_);
 }
...
void MixedInitiativeControllerDeepgaze::computeCostCallback(const ros::TimerEvent&)
{
        ...
        if (mi_active_ == 1)
        //if (mi_active_ == 1 and task_intent_active_ == 1)
        {
                ROS_INFO("bike");
                ...
                else if (count_timesteps_error_ > number_timesteps_error_)
                {
                        ...
                        engine_->setInputValue("operator_intent", operator_intent_.data);
...
//                        FL_LOG("speed = " << fl::Op::str(cmd_vel_robot_.linear.x) );
                        FL_LOG("operator_intent = " << fl::Op::str(operator_intent_.data) );
...
int main(int argc, char *argv[])
{
        ...
        fl::InputVariable* operator_intent = new fl::InputVariable;
        operator_intent->setName("operator_intent");
        operator_intent->setDescription("");
        operator_intent->setEnabled(true);
        operator_intent->setRange(-0.200, 1.200);
        operator_intent->setLockValueInRange(false);
        operator_intent->addTerm(new fl::Rectangle("navigating", -0.100, 0.100));
        operator_intent->addTerm(new fl::Rectangle("exploring", 0.900, 1.100));
        engine->addInputVariable(operator_intent);
...
//        inputVariable3->setName("task_intent");
...
// new rulebase ijrr + cogn av
        ...
        ruleBlock->addRule(fl::Rule::parse("if current_loa is teleop and operator_intent is exploring then change_LOA is no_change", engine));
...
//      ruleBlock->addRule(fl::Rule::parse("if error is small and speed is reverse and current_loa is teleop and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is reverse and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is reverse and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is reverse and current_loa is auto and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is zero and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is zero and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is zero and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is zero and current_loa is auto and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is forward and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is forward and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is forward and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is small and speed is forward and current_loa is auto and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is reverse and current_loa is teleop and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is reverse and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is reverse and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is reverse and current_loa is auto and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is zero and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is zero and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is zero and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is zero and current_loa is auto and task_intent is available then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is forward and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is forward and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is forward and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is medium and speed is forward and current_loa is auto and task_intent is available then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is reverse and current_loa is teleop and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is reverse and current_loa is teleop and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is reverse and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is reverse and current_loa is auto and task_intent is available then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is zero and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is zero and current_loa is teleop and task_intent is available then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is zero and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is zero and current_loa is auto and task_intent is available then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is forward and current_loa is teleop and task_intent is unavailable then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is forward and current_loa is teleop and task_intent is available then change_LOA is change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is forward and current_loa is auto and task_intent is unavailable then change_LOA is no_change", engine));
//	ruleBlock->addRule(fl::Rule::parse("if error is large and speed is forward and current_loa is auto and task_intent is available then change_LOA is change", engine));
      engine->addRuleBlock(ruleBlock);

//previous rule base -- without all combinations
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is unavailable and current_loa is teleop then change_LOA is change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is unavailable and current_loa is auto then change_LOA is no_change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is auto and error is large then change_LOA is change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is teleop and error is small then change_LOA is no_change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is teleop and error is medium then change_LOA is no_change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is teleop and error is large and speed is forward then change_LOA is change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is teleop and error is large and speed is zero then change_LOA is change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available and current_loa is teleop and speed is reverse and error is large then change_LOA is no_change", engine));
...
//        ruleBlock->addRule(fl::Rule::parse("if speed is reverse and error is large then change_LOA is no_change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is not_available then change_LOA is change", engine));
//        ruleBlock->addRule(fl::Rule::parse("if task_intent is available then change_LOA is no_change", engine));
//        engine->addRuleBlock(ruleBlock);
